...
def analyze_condition(request, policy_engine, ctx=None):
    norm = normalize(request.raw_value)

    # ---- 0) load canon defs (SoT) ----
    canon_defs = _load_canon_defs("conditions_canon_v1")  # reads .tsv via loader
    ...
    return AnalysisResult(
        ...
        canonical_value=top.value,   # ★ always canonical (e.g., 美品 / 新品 / 中古)
