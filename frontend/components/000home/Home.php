...
export default function Home() {
  ...
  /**
   * 🔐 認証済み API 用 fetcher
   * - isReady=false の間は絶対に使わない
   */
  const authed = isReady ? useAuthedFetcher() : null;
  ...
  useEffect(() => {
    if (!isAuthenticated || !authed) return;

    let cancelled = false;

    (async () => {
      try {
        const data = await authed.get<{ has_profile: boolean }>(
          "/mypage/profile"
        );
        if (cancelled) return;
  ...
  const isPageLoading = authLoading || isItemsLoading || isGateLoading;
  ...
  const toggleFavorite = async (item: PublicItem, isFavorited: boolean) => {
    ...
    try {
      if (isFavorited) {
        await authed.delete(`/reactions/items/${item.id}/favorite`);
      } else {
        await authed.post(`/reactions/items/${item.id}/favorite`);
      }
